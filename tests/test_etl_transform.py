...
class TestTransformDataOutput:
    ...
    def test_output_columns(self) -> None:
        ...
        expected = {
            "raw_name", "total_capacity_gb", "ddr_gen", "speed_mts",
            "has_rgb", "kit_modules", "brand", "series", "part_number",
            "price", "store",
        }
        assert set(result.columns) == expected

    @pytest.mark.parametrize("kit_string, expected_modules", [
        ("2x16GB", 2),
        ("(2x32GB)", 2),
        ("8GB", 1),
        ("16x2GB", 2)
    ...
    def test_kit_modules(self, kit_string: str, expected_modules: int) -> None:
        df = _make_raw_df([
            {"name": f"KINGSTON FURY {kit_string} DDR5 6000MHZ", "price": 179.99, "store": "cyc"},
        ])
        result = transform_data(df)
        assert result.iloc[0]["kit_modules"] == expected_modules

    @pytest.mark.parametrize("ddr_string, expected_ddr_gen", [
        ("DDR3", 3),
        ("DDR4", 4),
        ("DDR5", 5),  
    ])
    def test_ddr_gen_extraction(self, ddr_string: str, expected_ddr_gen: int) -> None:
        df = _make_raw_df([
            {"name": f"KINGSTON FURY 16GB {ddr_string} 6000MHZ", "price": 89.99, "store": "cyc"},
        ])
        result = transform_data(df)
        assert result.iloc[0]["ddr_gen"] == expected_ddr_gen

    @pytest.mark.parametrize("speed_string, expected_speed", [
        ("5200MHz", 5200),
        ("5200Mhz", 5200),
        ("5200MHZ", 5200),
        ("5200MT/S", 5200),
    ...
    @pytest.mark.parametrize("capacity_string, expected_capacity", [
        ("16GB", 16),
        ("24GB", 24),
        ("32GB", 32),
        ("48GB", 48)
    ...
    def test_capacity_extraction(self, capacity_string: str, expected_capacity: int) -> None:
        df = _make_raw_df([
            {"name": f"KINGSTON FURY {capacity_string} DDR5 6000MHZ", "price": 159.99, "store": "cyc"},
        ])
        result = transform_data(df)
